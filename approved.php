...
if ($_SESSION['ROLE']==1) {
    $sql="SELECT policy_tbl.*, users.firstname,category.category,policy_type.policy_type,users.id as eid FROM policy_tbl,category,policy_type, users where policy_tbl.policy_status=2 and policy_tbl.user_id=users.id and policy_tbl.policy_type=policy_type.id  order by policy_tbl.id desc";
    //$sql="SELECT * FROM policy_tbl order by id desc";
}else {
    $eid=$_SESSION['USER_ID'];
    $sql="SELECT policy_tbl.*, users.firstname,category.category,policy_type.policy_type,users.id as eid FROM policy_tbl,category,policy_type, users where policy_tbl.user_id='$eid' and policy_tbl.policy_status=2  and policy_tbl.user_id=users.id and policy_tbl.policy_type=policy_type.id order by policy_tbl.id desc";
}
...
.box-content{
    cursor: pointer;
                ...
                <div class="table-heads">
                        <h3 class="table-title">Approved Policies</h3>
                    ...
                    <table class="table">
                                <thead>
                                    <tr>
                                            ...
                                            <th width="5%">Ins Category</th>
                                            <th width="5%">Policy Name(policy_id)</th>
